...
20. **Overriding Docker defaults unnecessarily** → The Dockerfile has sensible defaults (NUM_WORKERS=10, DOCLING_DEVICE=cuda, etc). When running the container, use defaults: `docker run -d --name docling --gpus all -p 8000:8000 bhosh/docling-serve-patched:v9`. Do NOT pass `-e NUM_WORKERS=5` or other env vars unless you specifically need to change them.
21. **Filing bugs without reading API docs/docstrings** → Before blaming the API for an error, READ the endpoint's docstring in controller.py or check /openapi.json. Don't guess field names. Example: `/v1/convert/source/async` expects `file_id` for uploaded files and `source` for URLs - these are DIFFERENT fields. If you use the wrong field name, the error is YOUR fault, not a bug.
